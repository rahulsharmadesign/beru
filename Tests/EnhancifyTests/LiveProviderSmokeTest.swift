...
final class LiveProviderSmokeTestCase: XCTestCase {
    func testStreamingAgainstLocalServerProducesGrammarCorrection() async throws {
        let baseURL = ProcessInfo.processInfo.environment["ENHANCIFY_TEST_BASE_URL"] ?? "http://localhost:1234/v1"
        let model = ProcessInfo.processInfo.environment["ENHANCIFY_TEST_MODEL"] ?? "qwen/qwen3-1.7b"
