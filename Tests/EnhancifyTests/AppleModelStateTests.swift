...
// adapted to Enhancify's XCTest suite (the spike used Swift Testing). No UI, no
...
final class AppleModelStateTests: XCTestCase {
    ...
    func testNoStateRoutesToConnectToModel() {
        // `.modelUnavailable` drives Enhancify's "Connect to model" offer, which makes
    ...
    func testModelIDIsStableForUsageAttribution() {
        ...
        let suiteName = "enhancify.tests.\(UUID().uuidString)"
