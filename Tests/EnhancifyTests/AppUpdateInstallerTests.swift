...
final class AppUpdateInstallerTests: XCTestCase {
    ...
    private func payload(leaf: String? = "Developer ID Application: Example") -> AppUpdateInstaller.Payload {
        AppUpdateInstaller.Payload(
            dmgPath: "/tmp/Enhancify-update.dmg",
            destinationPath: "/Applications/Enhancify.app",
            bundleIdentifier: "com.rahul.enhancify",
    ...
    func testScriptChecksBundleIdentifier() {
        let script = AppUpdateInstaller.script(for: payload())
        XCTAssertTrue(script.contains("WANT_ID='com.rahul.enhancify'"))
    ...
    func testValuesAreSingleQuotedSoTheScriptCannotBeBroken() {
        let script = AppUpdateInstaller.script(for: payload(leaf: "Bad\" $(rm -rf ~) 'name"))
        XCTAssertTrue(script.contains("WANT_LEAF='Bad\" $(rm -rf ~) '\\''name'"))
        XCTAssertEqual(AppUpdateInstaller.shellQuoted("/Applications/My App.app"), "'/Applications/My App.app'")
    }
