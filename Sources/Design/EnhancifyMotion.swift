...
private struct EnhancifyParentHoveredKey: EnvironmentKey {
    static let defaultValue = false
}

extension EnvironmentValues {
    var enhancifyParentHovered: Bool {
        get { self[EnhancifyParentHoveredKey.self] }
        set { self[EnhancifyParentHoveredKey.self] = newValue }
    }
}

private struct EnhancifyEase: ViewModifier {
...
extension View {
    ...
    func enhancifyHoverEase(_ isHovered: Bool) -> some View {
        modifier(EnhancifyEase(value: isHovered, duration: 0.12))
    }

    /// Chip selection + highlight travel. Scoped to the chip so the panel
    /// window does not animate its height on a tab change.
    func enhancifyTabSwitchEase(_ isSelected: Bool) -> some View {
        modifier(EnhancifyEase(value: isSelected, animation: EnhancifyMotion.tabSwitchAnimation))
    }

    /// Eases focus rings and glows.
    func enhancifyFocusEase(_ isFocused: Bool) -> some View {
        modifier(EnhancifyEase(value: isFocused, duration: 0.15))
    }
}
