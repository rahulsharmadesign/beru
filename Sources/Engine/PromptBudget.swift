...
///
/// Enhancify caps a capture at `PanelEngine.maxCapturedLength` (8000 characters)
...
/// Enhancify's own system prompts are already ~700 of them. Without this clamp the
...
struct PromptBudget: Equatable, Sendable {
    ...
    /// Enhancify's own capture cap, so this clamp can never widen a capture.
...
/// Turns cumulative snapshots from Foundation Models into the deltas Enhancify's
/// stream consumers append.
///
/// `ResponseStream<String>.Snapshot.content` is the text *so far*, not the new
/// piece. Enhancify's `StreamChunk.content` is treated as a delta everywhere
