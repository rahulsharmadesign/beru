...
extension KeyboardShortcuts.Name {
    static let invokeEnhancify = Self("invokeEnhancify", default: .init(.p, modifiers: [.control, .option, .command]))
    /// Toggle dictation. Defaults to Control-Option-Command-L. Opens the panel
    /// and starts listening. Press again to stop.
    static let dictateToEnhancify = Self("dictateToEnhancify", default: .init(.l, modifiers: [.control, .option, .command]))
}

@main
struct EnhancifyApp: App {
...
struct MenuBarContent: View {
    ...
    private let rowHeight: CGFloat = EnhancifyMetrics.pillHeight
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: EnhancifySpace.hair) {
            header
            Divider().padding(.vertical, EnhancifySpace.xxs)
            ...
            menuRow(
                "Dictate",
                icon: "mic",
                shortcut: KeyboardShortcuts.getShortcut(for: .dictateToEnhancify)?.description
            ) {
        ...
        .frame(width: EnhancifyMetrics.menuDropdownWidth)
        ...
        // material.
        .background(EnhancifyColor.glassScrim)
    }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text("Enhancify").font(EnhancifyType.controlSemibold)
                Spacer(minLength: EnhancifySpace.xs)
                if let shortcut = KeyboardShortcuts.getShortcut(for: .invokeEnhancify) {
                    EnhancifyKbd(text: shortcut.description)
                }
            }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    static let showDashboardNotification = Notification.Name("com.rahul.enhancify.showDashboard")
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let others = NSRunningApplication.runningApplications(
            withBundleIdentifier: Bundle.main.bundleIdentifier ?? "com.rahul.enhancify"
        ).filter { app in
    ...
    /// as "Enhancify doesn't open when I click it in the Dock", which is
